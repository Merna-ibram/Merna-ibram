...
class CommissionLineMixin(models.AbstractModel):
    ...
    def _get_new_customer_commission_amount(self, commission, subtotal, product, quantity, newcustomer):
        ...
        if commission.commission_type == "fixed":
            ...
            # fix: need to chnage here
            return commission.calculate_section_new(subtotal, newcustomer)
    ...
    def _compute_commission_id(self):
        for record in self:
            # i think we can manage from here commission id
            record.commission_id = record.agent_id.commission_id
    @api.onchange('agent_id')
    def _onchange_agent_id(self):
        print("Self records:", self)
        for record in self:

            if record.object_id.move_id.state == 'posted' and not self.env.user.has_group(
                    'commission.group_edit_delete_commission'):
                print("Condition met - raising ValidationError")
                raise ValidationError(_("You do not have the necessary permissions to edit this record."))
            else:
                return

    def unlink(self):
        print("unlink")
        if self.object_id.move_id.state == 'posted' and not self.env.user.has_group(
                'commission.group_edit_delete_commission'):
            raise ValidationError(_("You do not have the necessary permissions to delete this record."))
        return super(CommissionLineMixin, self).unlink()
